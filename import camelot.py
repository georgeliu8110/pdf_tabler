...
# pdf_path = r"pdf convertion testing.pdf"
# output_excel = "111111111111111111111.xlsx"
...
#     # Normalize currency like "$ 3,231" or "$   (1,234)"
#     if "$" in s:
#         s = re.sub(r"\$\s+", "$", s)
#         s_num = s.replace("$", "").replace(",", "").strip()
...
# def merge_dollar_into_number_cell(df):
#     """
#     If a cell contains only '$' and the next cell contains a number,
#     move the merged currency text into the number cell (right cell),
#     then blank out the standalone '$' cell.
#     """
#     df = df.copy().astype(object)

#     for r in range(df.shape[0]):
#         c = 0
#         while c < df.shape[1] - 1:
#             current_val = "" if pd.isna(df.iat[r, c]) else str(df.iat[r, c]).strip()
#             next_val = "" if pd.isna(df.iat[r, c + 1]) else str(df.iat[r, c + 1]).strip()

#             if current_val == "$" and next_val:
#                 if re.fullmatch(r"\(?-?\d[\d,]*(\.\d+)?\)?", next_val):
#                     # keep the value in the number cell
#                     df.iat[r, c] = ""
#                     df.iat[r, c + 1] = f"${next_val}"
...
#     """
#     Delete columns that are completely blank after cleanup.
#     """
#     df = df.copy()
...
#                     # Merge standalone $ into the number cell on the right
#                     df = merge_dollar_into_number_cell(df)

#                     # Remove columns that became fully blank
...
#                             excel_row = start_row + r + 2  # +1 for header row in Excel, +1 because Excel starts at 1
#                             excel_col = c + 1
...
#                     # Write each table below the previous one
#                     df.to_excel(
#                         writer,
#                         sheet_name=sheet_name,
#                         index=False,
